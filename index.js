...
module.exports = function (opt, callback) {
  ...
  opt.dir = opt.dir.filter(d => Object.prototype.toString.call(d) === '[object String]' && d.trim() && /^\//.test(d));
  if (!opt.dir.length) {
    console.log('Charactor Scanner :: ERR: All dirs need to be Absolute paths, please use [path.resolve] to access them before pass');
    return;
  }
